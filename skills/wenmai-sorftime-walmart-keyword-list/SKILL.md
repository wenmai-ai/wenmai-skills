---
name: wenmai-sorftime-walmart-keyword-list
description: "Sorftime Walmart 实时热搜关键词列表接口，通过固定 Wenmai standard API `walmart_keyword_list` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要实时热搜关键词列表或调用该接口时触发。即使用户未明确提及 Sorftime，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# Sorftime walmart_keyword_list

## Overview

调用 Wenmai Sorftime standard API `walmart_keyword_list`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sorftime/walmart-keyword-list`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/walmart_keyword_list.py`
...
python3 scripts/walmart_keyword_list.py '{"rank_min":1,"rank_max":100}'
```
