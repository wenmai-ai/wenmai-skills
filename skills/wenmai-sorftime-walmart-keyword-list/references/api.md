# Wenmai Sorftime `walmart_keyword_list` API 参考

实时热搜关键词列表。

## 调用规范

- **standard API 地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sorftime/walmart-keyword-list`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`walmart_keyword_list`
- **脚本入口**：`scripts/walmart_keyword_list.py`
...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `rank_min` | Number | 是 | 排名范围的起始值（如 1）。 |
| `rank_max` | Number | 是 | 排名范围的结束值（如 100）。 |
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `data.images[]` | Array | 该关键词对应的代表性商品图片 URL 数组（首页商品主图）。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sorftime/walmart-keyword-list" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"rank_min":1,"rank_max":100}'
```
