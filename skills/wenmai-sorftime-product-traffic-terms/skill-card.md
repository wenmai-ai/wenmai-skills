...
Sorftime Amazon（亚马逊） 产品流量词反查接口，通过固定 Wenmai standard API `product_traffic_terms` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `product_traffic_terms`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Amazon（亚马逊）
- Operation: `product_traffic_terms`
