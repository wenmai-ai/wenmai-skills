...
description: "Use when users need the Wenmai Alpha standard API `alpha_xiaohongshu_pro_scraper` for Xiaohongshu Pro Scraper, including 小红书笔记、关键词和内容趋势. Sends user parameters as a JSON POST body to `/wmapi/v1/alpha/xiaohongshu-pro-scraper` and returns traceable raw gateway data for ecommerce research, competitor discovery, VOC, social trend analysis, or reporting."
---

# Wenmai Alpha Xiaohongshu Pro Scraper

## Purpose
Use this Skill to fetch Xiaohongshu Pro Scraper data through Wenmai standard API. It is useful for 小红书笔记、关键词和内容趋势.

The Skill calls exactly one fixed standard API endpoint:

- Endpoint: `POST /wmapi/v1/alpha/xiaohongshu-pro-scraper`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/alpha_xiaohongshu_pro_scraper.py`
...
python3 scripts/alpha_xiaohongshu_pro_scraper.py '{"mode": "search", "keywords": ["AI"], "noteType": "不限", "sortType": "general", "timeFilter": "不限", "fetchComments": false, "maxItemsPerInput": 3}'
```
...
Treat `mode: "notes"` with `noteUrls` as an opt-in, separately billed full-note-detail capability. Treat `fetchComments: true` as additional paid detail processing. Do not infer or enable either capability automatically. Include it only when the user explicitly requests full note details or comments, and tell the user before the request that it is billed separately.
