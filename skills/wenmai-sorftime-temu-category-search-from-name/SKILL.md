---
name: wenmai-sorftime-temu-category-search-from-name
description: "Sorftime Temu 按产品名称搜类目接口，通过固定 Wenmai standard API `temu_category_search_from_name` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要按产品名称搜类目或调用该接口时触发。即使用户未明确提及 Sorftime，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# Sorftime temu_category_search_from_name

## Overview

调用 Wenmai Sorftime standard API `temu_category_search_from_name`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sorftime/temu-category-search-from-name`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/temu_category_search_from_name.py`
...
python3 scripts/temu_category_search_from_name.py '{"name":"wireless earbuds","site":"US"}'
```
