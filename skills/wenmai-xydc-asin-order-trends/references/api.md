# Wenmai XYDC asin订单量趋势（月） API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-asin-order-trends`
...
- **接口编码**：`get_asin_order_trends`
- **接口说明**：asin订单量趋势（月）。
- **脚本入口**：`scripts/xydc_asin_order_trends.py`，脚本参数即标准 API POST Body JSON
...
| `data.trends[].date` | string | 趋势开始月份（YYYY-MM格式）。 |
| `data.trends[].orders` | integer | 订单量。 |

## 使用要点

- 本 Skill 直接调用 Wenmai 标准 API `/xydc/get-asin-order-trends`。
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-asin-order-trends" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"asin": "B09PCSR9SX", "country": "US", "start_month": "2026-05", "end_month": "2026-06"}'
```

---
来源：Wenmai WMAPI 文档 https://all-api.wenmai-ai.com/wmapi/docs#xydc-get_asin_order_trends（2026-07-07 访问）。
