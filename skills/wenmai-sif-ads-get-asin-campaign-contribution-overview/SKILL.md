---
name: wenmai-sif-ads-get-asin-campaign-contribution-overview
description: "SIF Amazon（亚马逊） 功能：基于曝光得分，查询某 ASIN 在指定时间窗口内各 campaign 的贡献总览，按贡献从高到低排序接口，通过固定 Wenmai standard API `ads_get_asin_campaign_contribution_overview` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要功能：基于曝光得分，查询某 ASIN 在指定时间窗口内各 campaign 的贡献总览，按贡献从高到低排序或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# SIF ads_get_asin_campaign_contribution_overview

## Overview

调用 Wenmai SIF standard API `ads_get_asin_campaign_contribution_overview`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sif/ads-get-asin-campaign-contribution-overview`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/ads_get_asin_campaign_contribution_overview.py`
...
python3 scripts/ads_get_asin_campaign_contribution_overview.py '{"asin":"B08GHW4TBS","country":"US","end_date":"2026-04-04","start_date":"2026-03-29"}'
```
