# Wenmai Sorftime `category_search_from_top_node` API 参考

查大类下的细分类目市场。

## 调用规范

- **standard API 地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sorftime/category-search-from-top-node`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`category_search_from_top_node`
- **脚本入口**：`scripts/category_search_from_top_node.py`
...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `top_node` | String | 是 | 要搜索的顶级类目（如 automotive、home & kitchen）；需先通过 category_tree 获取顶级 node id 或名称。 |
...
| `price_min` | Number | 否 | 可选：筛选平均售价（本地货币，如 US 站为 USD、FR 站为 EUR）大于等于该值的子类目市场。 |
...
| `top3_product_sales_share_min` | Number | 否 | 可选：用 0~1 表示百分比（如 10% = 0.1）。筛选 Top-100 商品中 Top-3 商品销量占比（销量集中度）大于等于该值的子类目市场。 |
| `top3_product_sales_share_max` | Number | 否 | 可选：用 0~1 表示百分比。筛选 Top-100 商品中 Top-3 商品销量占比小于等于该值的子类目市场。 |
| `amazon_owned_sales_share_min` | Number | 否 | 可选：用 0~1 表示百分比。筛选 Top-100 商品中 Amazon 自营销量占比（卖家集中度）大于等于该值的子类目市场。 |
| `amazon_owned_sales_share_max` | Number | 否 | 可选：用 0~1 表示百分比。筛选 Top-100 商品中 Amazon 自营销量占比小于等于该值的子类目市场。 |
| `top100_top400_sales_share_min` | Number | 否 | 可选：用 0~1 表示百分比。筛选 Top-100 商品销量占 Top-400 的比例（值越低代表长尾越深）大于等于该值的子类目市场。 |
| `top100_top400_sales_share_max` | Number | 否 | 可选：用 0~1 表示百分比。筛选 Top-100 商品销量占 Top-400 的比例小于等于该值的子类目市场。 |
| `newproduct_sales_share_min` | Number | 否 | 可选：用 0~1 表示百分比。筛选 Top-100 商品中上架 3 个月内新品销量占比大于等于该值的子类目市场。 |
| `newproduct_sales_share_max` | Number | 否 | 可选：用 0~1 表示百分比。筛选 Top-100 商品中上架 3 个月内新品销量占比小于等于该值的子类目市场。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sorftime/category-search-from-top-node" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"top_node":"Home & Kitchen","amz_site":"US"}'
```
