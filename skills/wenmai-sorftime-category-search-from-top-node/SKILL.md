---
name: wenmai-sorftime-category-search-from-top-node
description: "Sorftime Amazon（亚马逊） 查大类下的细分类目市场接口，通过固定 Wenmai standard API `category_search_from_top_node` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要查大类下的细分类目市场或调用该接口时触发。即使用户未明确提及 Sorftime，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# Sorftime Amazon（亚马逊） category_search_from_top_node

## Overview

调用 Wenmai Sorftime standard API `category_search_from_top_node`。只调用以下固定端点，不接受动态端点或其他操作：

- Platform: Amazon（亚马逊）
- Endpoint: `POST /wmapi/v1/sorftime/category-search-from-top-node`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/category_search_from_top_node.py`
...
python3 scripts/category_search_from_top_node.py '{"top_node":"Home & Kitchen","amz_site":"US"}'
```
