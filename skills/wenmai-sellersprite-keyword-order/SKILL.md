---
name: wenmai-sellersprite-keyword-order
description: "卖家精灵（SellerSprite） Amazon（亚马逊） 基于 ASIN 的关键词反查工具，用于分析某个或多个 ASIN接口，通过固定 Wenmai standard API `keyword_order` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要基于 ASIN 的关键词反查工具，用于分析某个或多个 ASIN或调用该接口时触发。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 卖家精灵（SellerSprite） standard API `keyword_order`。只调用以下固定端点，不接受动态端点或其他操作：
...
python3 scripts/keyword_order.py '{"request":{"date":"2026-03-29","asins":["B08GHW4TBS"],"marketplace":"US","reverseType":"W"}}'
```
