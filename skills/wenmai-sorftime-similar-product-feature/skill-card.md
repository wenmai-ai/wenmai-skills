...
Sorftime Amazon（亚马逊） 相似产品特征接口，通过固定 Wenmai standard API `similar_product_feature` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `similar_product_feature`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Amazon（亚马逊）
- Operation: `similar_product_feature`
