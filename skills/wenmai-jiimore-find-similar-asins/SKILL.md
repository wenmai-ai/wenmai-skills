...
description: "极目数据（JIIMORE）Amazon ABA 相似商品接口，支持输入 1～20 个 ASIN，返回相似商品的标题、图片、品牌、价格、评分、评论量、点击与转化、卖家、ABA 相似度和相关等级。用于相似 ASIN、替代商品、关联竞品或竞品样本扩展。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# 极目数据（JIIMORE）Amazon ABA 相似商品

## Overview

调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_find_similar_asins`。只调用以下固定端点，不接受动态端点或其他操作：
...
- 按 ASIN 展示商品信息、价格、评分/评论、点击与转化、卖家、ABA 相似度和相关等级。
