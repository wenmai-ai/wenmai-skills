...
极目数据（JIIMORE）Amazon ABA 相似商品接口，支持输入 1～20 个 ASIN，返回相似商品的标题、图片、品牌、价格、评分、评论量、点击与转化、卖家、ABA 相似度和相关等级。

固定调用 Wenmai 极目数据（JIIMORE） standard API `jiimore_find_similar_asins`，不接受动态端点或其他操作。

- Provider: 极目数据（JIIMORE）
- Platform: Amazon（亚马逊）
- Operation: `jiimore_find_similar_asins`
