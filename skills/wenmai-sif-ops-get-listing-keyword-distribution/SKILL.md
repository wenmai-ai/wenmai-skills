---
name: wenmai-sif-ops-get-listing-keyword-distribution
description: "SIF Amazon（亚马逊） 功能：查看各变体的关键词数量分布，返回每个变体在自然流量、SP、SB、SBV 各渠道中覆盖的流量词数量接口，通过固定 Wenmai standard API `ops_get_listing_keyword_distribution` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要功能：查看各变体的关键词数量分布，返回每个变体在自然流量、SP、SB、SBV 各渠道中覆盖的流量词数量或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# SIF ops_get_listing_keyword_distribution

## Overview

调用 Wenmai SIF standard API `ops_get_listing_keyword_distribution`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sif/ops-get-listing-keyword-distribution`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/ops_get_listing_keyword_distribution.py`
...
python3 scripts/ops_get_listing_keyword_distribution.py '{"asin":"B08GHW4TBS"}'
```
