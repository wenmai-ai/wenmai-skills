...
卖家精灵（SellerSprite） Amazon（亚马逊） 用于在指定 Amazon 站点和时间点（按月），接口，通过固定 Wenmai standard API `aba_research_monthly` 返回可追溯的原始网关数据。

固定调用 Wenmai 卖家精灵（SellerSprite） standard API `aba_research_monthly`，不接受动态端点或其他操作。
