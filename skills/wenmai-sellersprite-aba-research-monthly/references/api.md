# Wenmai SellerSprite `aba_research_monthly` API 参考

用于在指定 Amazon 站点和时间点（按月），。

## 调用规范

- **standard API 地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sellersprite/aba-research-monthly`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`aba_research_monthly`
- **脚本入口**：`scripts/aba_research_monthly.py`
...
| `request.date` | string | 否 | 为空时，查最近30天；示例：202506。 |
...
| `request.size` | integer | 否 | 每页条数，最大15；默认：15。 |
...
| `items[].date` | string | 查询日期；官方示例值：202306。 |
...
| `items[].w1SearchRank` | integer | 上月的排名。 |
| `items[].w1RankGrowthValue` | integer | 上月的排名变化值。 |
| `items[].w1RankGrowthRate` | number | 上月的排名变化率。 |
| `items[].w4SearchRank` | integer | 4月前的排名。 |
| `items[].w4RankGrowthValue` | integer | 4月前的排名变化值。 |
| `items[].w4RankGrowthRate` | number | 4月前的排名变化率。 |
| `items[].w12SearchRank` | integer | 12月前的排名。 |
| `items[].w12RankGrowthValue` | integer | 12月前的排名变化值。 |
| `items[].w12RankGrowthRate` | number | 12月前的排名变化率。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sellersprite/aba-research-monthly" \
