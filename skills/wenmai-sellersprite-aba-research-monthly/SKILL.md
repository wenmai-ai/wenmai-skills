---
name: wenmai-sellersprite-aba-research-monthly
description: "卖家精灵（SellerSprite） Amazon（亚马逊） 用于在指定 Amazon 站点和时间点（按月），接口，通过固定 Wenmai standard API `aba_research_monthly` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要用于在指定 Amazon 站点和时间点（按月），或调用该接口时触发。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# 卖家精灵（SellerSprite） aba_research_monthly

## Overview

调用 Wenmai 卖家精灵（SellerSprite） standard API `aba_research_monthly`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sellersprite/aba-research-monthly`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/aba_research_monthly.py`
...
python3 scripts/aba_research_monthly.py '{"request":{"marketplace":"US"}}'
```
