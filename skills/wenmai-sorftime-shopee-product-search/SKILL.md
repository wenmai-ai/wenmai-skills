---
name: wenmai-sorftime-shopee-product-search
description: "Sorftime Shopee 选产品接口，通过固定 Wenmai standard API `shopee_product_search` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要选产品或调用该接口时触发。即使用户未明确提及 Sorftime，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# Sorftime shopee_product_search

## Overview

调用 Wenmai Sorftime standard API `shopee_product_search`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sorftime/shopee-product-search`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/shopee_product_search.py`
...
python3 scripts/shopee_product_search.py '{"site":"TH"}'
```
