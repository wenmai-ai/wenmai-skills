---
name: wenmai-sellersprite-bsr-prediction
description: "卖家精灵（SellerSprite） Amazon（亚马逊） 根据 Amazon 指定市场下的一级类目节点和大类 BSR 排名，接口，通过固定 Wenmai standard API `bsr_prediction` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要根据 Amazon 指定市场下的一级类目节点和大类 BSR 排名，或调用该接口时触发。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 卖家精灵（SellerSprite） standard API `bsr_prediction`。只调用以下固定端点，不接受动态端点或其他操作：
...
python3 scripts/bsr_prediction.py '{"bsr":1,"categoryId":"11260432011","marketplace":"US"}'
```
