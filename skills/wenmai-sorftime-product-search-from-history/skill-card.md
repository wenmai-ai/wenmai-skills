...
Sorftime Amazon（亚马逊） 选产品（历史）接口，通过固定 Wenmai standard API `product_search_from_history` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `product_search_from_history`，不接受动态端点或其他操作。
