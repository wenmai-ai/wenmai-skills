# Wenmai JIIMORE jiimore_get_niche_keywords API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/jiimore/niche-keywords`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，也兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`jiimore_get_niche_keywords`
- **脚本入口**：`scripts/jiimore_get_niche_keywords.py`
...
| `items[].clickShare90d` | number | 关键词在细分市场中的 90 日点击份额。 |
| `items[].searchConversionRate90d` | number | 90 日搜索转化率。 |
| `items[].keywordClickCount90d` | integer | 关键词 90 日点击量。 |
| `items[].nicheClickCount90d` | integer | 细分市场 90 日点击量。 |
| `items[].keywordSearchVolume90d` | integer | 关键词 90 日搜索量。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/jiimore/niche-keywords" \
