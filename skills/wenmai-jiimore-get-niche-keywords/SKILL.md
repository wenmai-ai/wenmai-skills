...
description: "极目数据（JIIMORE）Amazon 细分市场关键词接口，按 niche ID 返回搜索词、搜索量、点击、销量、转化、CPC/CPR、ABA 排名变化、集中度及关键词在细分市场中的 90 日点击/搜索份额。用于 niche 关键词池、流量词、搜索需求、词份额或赛道关键词分析。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_get_niche_keywords`。只调用以下固定端点，不接受动态端点或其他操作：
...
- 按搜索词展示量级、转化、CPC/CPR、ABA 排名变化、集中度，以及 90 日关键词/细分市场点击量、搜索量和份额。
