# Wenmai Alpha Shein Scraper API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/shein-scraper`
...
- **接口编码**：`alpha_shein_scraper`
- **接口说明**：Shein Product Scraper
- **脚本入口**：`scripts/alpha_shein_scraper.py`，脚本参数即标准 API POST Body JSON
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `country` | string | 否 | 国家或站点。可选值：US、UK、DE、FR、ES、IT、AU、BR、CA、MX、NL、SE、PL、CH、PT、AT、IL、PH、SG、MY、NZ、ZA。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/shein-scraper" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"country": "US", "maxItems": 3}'
```
