# Wenmai SellerSprite `market_ratings_count_distribution` API 参考

用于分析指定 Amazon 市场类目节点下的商品评分数区间分布与新品进入难度。。

## 调用规范

- **standard API 地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sellersprite/market-ratings-count-distribution`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`market_ratings_count_distribution`
- **脚本入口**：`scripts/market_ratings_count_distribution.py`
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `label` | string | 类型说明；官方示例值：500以上。 |
| `asins` | array | 包含的asin列表；官方示例值：5。 |
| `products` | integer | 产品数；官方示例值：["B00P19MFYE"]。 |
| `revenue` | number | 销售额；官方示例值：61714.24。 |
| `units` | integer | 销量；官方示例值：5616。 |
| `unitsRatio` | number | 销量占比；官方示例值：0.9743。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sellersprite/market-ratings-count-distribution" \
