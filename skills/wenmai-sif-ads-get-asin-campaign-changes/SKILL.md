---
name: wenmai-sif-ads-get-asin-campaign-changes
description: "SIF Amazon（亚马逊） 功能：查询某 ASIN 在历史各周内新上线的 campaign 变更事件，即 campaign_created 事件列表接口，通过固定 Wenmai standard API `ads_get_asin_campaign_changes` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要功能：查询某 ASIN 在历史各周内新上线的 campaign 变更事件，即 campaign_created 事件列表或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai SIF standard API `ads_get_asin_campaign_changes`。只调用以下固定端点，不接受动态端点或其他操作：
...
python3 scripts/ads_get_asin_campaign_changes.py '{"asin":"B08GHW4TBS","country":"US"}'
```
...
此操作会修改远程状态。执行前必须向用户复核最终完整请求，并确认全部目标/作用域字段：`asin`, `country`。可选目标字段未提供时，也必须确认采用上游默认作用域；未确认时不要调用。
