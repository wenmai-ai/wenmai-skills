# Wenmai SellerSprite `market_brand_concentration` API 参考

用于分析指定 Amazon 市场类目节点下的品牌集中度情况。。

## 调用规范

- **standard API 地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sellersprite/market-brand-concentration`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`market_brand_concentration`
- **脚本入口**：`scripts/market_brand_concentration.py`
...
| `asins` | array | 包含的商品ASIN集合；官方示例值：["B00P19MFYE"]。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sellersprite/market-brand-concentration" \
