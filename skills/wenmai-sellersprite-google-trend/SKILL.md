...
description: "卖家精灵（SellerSprite） Google Trends 关键词趋势接口，用于查询指定关键词在目标市场和时间范围内的搜索热度变化。当用户提到 Google 趋势、关键词热度、需求变化、季节性、市场趋势、关键词对比或 SellerSprite Google Trend 时触发此技能。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 卖家精灵（SellerSprite） standard API `google_trend`。只调用以下固定端点，不接受动态端点或其他操作：
