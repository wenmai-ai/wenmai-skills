...
description: "卖家精灵（SellerSprite） ASIN 流量关键词接口，用于查询关键词列表、自然位与广告位、搜索量、购买率、流量占比、关键词类型、转化类型和排名位置。当用户提到 ASIN 反查关键词、自然排名、广告排名、流量词、转化词、关键词占比或 SellerSprite traffic keyword 时触发此技能。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 卖家精灵（SellerSprite） standard API `traffic_keyword`。只调用以下固定端点，不接受动态端点或其他操作：
