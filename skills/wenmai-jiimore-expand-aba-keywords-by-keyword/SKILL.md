...
description: "极目数据（JIIMORE）Amazon ABA 关键词扩展接口，支持按 1～20 个种子词查询相关搜索词、7 日搜索/点击/销量与转化、CPC/CPR、ABA 排名变化、流量集中度、关联细分市场和竞争/广告成本信号。用于 ABA 扩词、关键词机会、搜索词增长、竞争度或广告成本分析。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_expand_aba_keywords_by_keyword`。只调用以下固定端点，不接受动态端点或其他操作：
...
- 按搜索词展示搜索量、点击量、销量、转化率、CPC/CPR、ABA 排名及变化、集中度、关联 niche、相关度、竞争度和广告成本信号。
