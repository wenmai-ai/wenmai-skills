---
name: wenmai-sorftime-walmart-product-variation-sales-by-product-id
description: "Sorftime Walmart 产品变体查询接口，通过固定 Wenmai standard API `walmart_product_variation_sales_by_product_id` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要产品变体查询或调用该接口时触发。即使用户未明确提及 Sorftime，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai Sorftime standard API `walmart_product_variation_sales_by_product_id`。只调用以下固定端点，不接受动态端点或其他操作：
...
python3 scripts/walmart_product_variation_sales_by_product_id.py '{"product_id":"11381374703"}'
```
