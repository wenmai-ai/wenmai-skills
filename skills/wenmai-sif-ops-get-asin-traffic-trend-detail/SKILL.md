---
name: wenmai-sif-ops-get-asin-traffic-trend-detail
description: "SIF Amazon（亚马逊） 功能：查看 ASIN 在指定时间窗口内的关键词级流量明细，按关键词分页返回各渠道排名与分数拆解接口，通过固定 Wenmai standard API `ops_get_asin_traffic_trend_detail` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要功能：查看 ASIN 在指定时间窗口内的关键词级流量明细，按关键词分页返回各渠道排名与分数拆解或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# SIF ops_get_asin_traffic_trend_detail

## Overview

调用 Wenmai SIF standard API `ops_get_asin_traffic_trend_detail`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sif/ops-get-asin-traffic-trend-detail`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/ops_get_asin_traffic_trend_detail.py`
...
python3 scripts/ops_get_asin_traffic_trend_detail.py '{"asin":"B08GHW4TBS","desc":false,"endDay":"2026-03-29","pageNum":1,"pageSize":1,"granularity":"day"}'
```
