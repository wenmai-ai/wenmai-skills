...
description: "极目数据（JIIMORE）Amazon ASIN 批量摘要接口，支持一次查询 1～20 个 ASIN，返回父子 ASIN、标题、图片、品牌、列表价格、评分、评论量、7/30/360 日点击与转化及卖家信息。用于批量商品核验、竞品清单补全、商品摘要或多 ASIN 对比。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_find_asins_batch`。只调用以下固定端点，不接受动态端点或其他操作：
...
- 按 ASIN 展示父 ASIN、标题、品牌、列表价格、评分/评论、点击与转化及卖家信息。
- 列表接口的 `currentPrice` 可能为 0；需要准确价格时改用 `wenmai-jiimore-get-asin-details`，不要把 0 解释为免费。
