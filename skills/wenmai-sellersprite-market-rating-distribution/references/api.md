# Wenmai SellerSprite `market_rating_distribution` API 参考

用于分析指定 Amazon 市场类目节点下的商品评分值分布与市场成熟度评估。。

## 调用规范

- **standard API 地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sellersprite/market-rating-distribution`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`market_rating_distribution`
- **脚本入口**：`scripts/market_rating_distribution.py`
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `label` | string | 类型说明；官方示例值：4.5以上。 |
| `asins` | array | 包含的asin列表；官方示例值：["B00P19MFYE"]。 |
| `products` | integer | 产品数；官方示例值：5。 |
| `revenue` | number | 销售额；官方示例值：59934.22。 |
| `units` | integer | 销量；官方示例值：5418。 |
| `unitsRatio` | number | 销量占比；官方示例值：0.9647。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sellersprite/market-rating-distribution" \
