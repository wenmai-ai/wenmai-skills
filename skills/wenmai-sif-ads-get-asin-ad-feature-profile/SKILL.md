---
name: wenmai-sif-ads-get-asin-ad-feature-profile
description: "SIF Amazon（亚马逊） 功能：ads_get_asin_ad_window_feature_profile 的兼容别名，执行逻辑完全相同接口，通过固定 Wenmai standard API `ads_get_asin_ad_feature_profile` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要功能：ads_get_asin_ad_window_feature_profile 的兼容别名，执行逻辑完全相同或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# SIF ads_get_asin_ad_feature_profile

## Overview

调用 Wenmai SIF standard API `ads_get_asin_ad_feature_profile`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sif/ads-get-asin-ad-feature-profile`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/ads_get_asin_ad_feature_profile.py`
...
python3 scripts/ads_get_asin_ad_feature_profile.py '{"asin":"B08GHW4TBS","country":"US","end_date":"2026-04-04","start_date":"2026-03-29"}'
```
