# Wenmai Alpha Walmart Reviews Scraper API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/walmart-reviews-scraper`
...
- **接口编码**：`alpha_walmart_reviews_scraper`
- **接口说明**：Walmart Reviews Scraper
- **脚本入口**：`scripts/alpha_walmart_reviews_scraper.py`，脚本参数即标准 API POST Body JSON
...
| `maxProductsPerStartUrl` | integer | 否 | 链接地址。 |
| `maxReviewsPerProduct` | integer | 否 | 最大采集或返回数量。 |
| `reviewsSortType` | string | 否 | 排序方式。可选值：relevancy、helpful、submission-desc、submission-asc、rating-desc、rating-asc。 |
| `scrapeUntilDate` | string | 否 | 时间或日期。 |
...
```json
{
  ...
  "scrapeUntilDate": "keyboard",
  "maxReviewsPerProduct": 3,
  "maxProductsPerStartUrl": 3
}
```
...
| `reviewText` | string | 评论内容。 |
| `reviewerName` | string | 评论者名称。 |
| `reviewDate` | string | 评论时间。 |
| `helpfulVotes` | number | 有帮助票数。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/walmart-reviews-scraper" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"startUrls": [{"url": "https://walmart.com/search?q=tshirt"}], "reviewsSortType": "relevancy", "scrapeUntilDate": "keyboard", "maxReviewsPerProduct": 3, "maxProductsPerStartUrl": 3}'
```
