# Wenmai SellerSprite `market_ebc_distribution` API 参考

用于分析指定 Amazon 市场类目节点下的A+视频分布。

## 调用规范

- **standard API 地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sellersprite/market-ebc-distribution`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`market_ebc_distribution`
- **脚本入口**：`scripts/market_ebc_distribution.py`
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `label` | string | 类型说明；官方示例值：有A+有视频。 |
| `products` | integer | 产品数；官方示例值：1。 |
| `productsRatio` | number | 类目名称产品占比；官方示例值：20。 |
| `units` | integer | 销量；官方示例值：1311。 |
| `unitsRatio` | number | 销量占比；官方示例值：23.34。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/sellersprite/market-ebc-distribution" \
