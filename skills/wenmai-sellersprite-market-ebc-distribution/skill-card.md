...
卖家精灵（SellerSprite） Amazon（亚马逊） 用于分析指定 Amazon 市场类目节点下的A+视频分布接口，通过固定 Wenmai standard API `market_ebc_distribution` 返回可追溯的原始网关数据。

固定调用 Wenmai 卖家精灵（SellerSprite） standard API `market_ebc_distribution`，不接受动态端点或其他操作。

- Provider: 卖家精灵（SellerSprite）
- Platform: Amazon（亚马逊）
- Operation: `market_ebc_distribution`
