...
Sorftime Amazon（亚马逊） 删除收藏关键词接口，通过固定 Wenmai standard API `del_favorite_keyword` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `del_favorite_keyword`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Amazon（亚马逊）
- Operation: `del_favorite_keyword`
