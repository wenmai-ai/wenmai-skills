...
Sorftime Amazon（亚马逊） 竞品关键词分析接口，通过固定 Wenmai standard API `competitor_product_keywords` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `competitor_product_keywords`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Amazon（亚马逊）
- Operation: `competitor_product_keywords`
