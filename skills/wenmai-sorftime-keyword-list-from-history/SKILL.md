---
name: wenmai-sorftime-keyword-list-from-history
description: "Sorftime Amazon（亚马逊） 历史热搜关键词榜接口，通过固定 Wenmai standard API `keyword_list_from_history` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要历史热搜关键词榜或调用该接口时触发。即使用户未明确提及 Sorftime，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# Sorftime Amazon（亚马逊） keyword_list_from_history

## Overview

调用 Wenmai Sorftime standard API `keyword_list_from_history`。只调用以下固定端点，不接受动态端点或其他操作：

- Platform: Amazon（亚马逊）
- Endpoint: `POST /wmapi/v1/sorftime/keyword-list-from-history`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/keyword_list_from_history.py`
...
python3 scripts/keyword_list_from_history.py '{"date":"2026-06-30","keyword_support_site":"US"}'
```
