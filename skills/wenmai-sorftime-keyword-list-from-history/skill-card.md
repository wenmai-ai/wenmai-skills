...
Sorftime Amazon（亚马逊） 历史热搜关键词榜接口，通过固定 Wenmai standard API `keyword_list_from_history` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `keyword_list_from_history`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Amazon（亚马逊）
- Operation: `keyword_list_from_history`
