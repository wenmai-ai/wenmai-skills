# Wenmai JIIMORE jiimore_find_asins_by_keyword API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/jiimore/asins-by-keyword`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，也兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`jiimore_find_asins_by_keyword`
- **脚本入口**：`scripts/jiimore_find_asins_by_keyword.py`
...
| `items[].currentPrice` | number | 当前价格；列表接口可能返回 0，准确价格应使用 ASIN 详情接口。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/jiimore/asins-by-keyword" \
