...
极目数据（JIIMORE）Amazon 关键词商品搜索接口，支持按 1～200 字符的搜索词查找 ASIN，并返回标题、图片、品牌、列表价格、评分、评论量、7/30/360 日点击与转化及卖家信息。

固定调用 Wenmai 极目数据（JIIMORE） standard API `jiimore_find_asins_by_keyword`，不接受动态端点或其他操作。

- Provider: 极目数据（JIIMORE）
- Platform: Amazon（亚马逊）
- Operation: `jiimore_find_asins_by_keyword`
