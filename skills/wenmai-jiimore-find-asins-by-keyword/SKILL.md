...
description: "极目数据（JIIMORE）Amazon 关键词商品搜索接口，支持按 1～200 字符的搜索词查找 ASIN，并返回标题、图片、品牌、列表价格、评分、评论量、7/30/360 日点击与转化及卖家信息。用于关键词搜品、候选商品池、按词找 ASIN、竞品发现或选品研究。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# 极目数据（JIIMORE）Amazon 关键词商品搜索

## Overview

调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_find_asins_by_keyword`。只调用以下固定端点，不接受动态端点或其他操作：
...
- 按 ASIN 展示商品信息、列表价格、评分/评论、点击与转化及卖家信息。
- 列表接口的 `currentPrice` 可能为 0；需要准确价格时改用 ASIN 详情接口。
