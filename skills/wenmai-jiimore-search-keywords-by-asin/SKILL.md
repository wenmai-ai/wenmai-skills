...
description: "极目数据（JIIMORE）Amazon ASIN 关联关键词接口，按单个 ASIN 返回搜索词、搜索量、点击、销量、搜索/点击转化、CPC/CPR、ABA 排名变化、增长率、Top 3 集中度和关联 niche。用于 ASIN 反查关键词、竞品关键词池、商品关联词或流量词发现。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_search_keywords_by_asin`。只调用以下固定端点，不接受动态端点或其他操作：
