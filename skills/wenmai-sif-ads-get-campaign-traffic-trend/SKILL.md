---
name: wenmai-sif-ads-get-campaign-traffic-trend
description: "SIF Amazon（亚马逊） 功能：查询单个 campaign 从创建至今的全生命周期流量趋势，并附带广告组创建事件作为结构性上下文接口，通过固定 Wenmai standard API `ads_get_campaign_traffic_trend` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要功能：查询单个 campaign 从创建至今的全生命周期流量趋势，并附带广告组创建事件作为结构性上下文或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# SIF ads_get_campaign_traffic_trend

## Overview

调用 Wenmai SIF standard API `ads_get_campaign_traffic_trend`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sif/ads-get-campaign-traffic-trend`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/ads_get_campaign_traffic_trend.py`
...
python3 scripts/ads_get_campaign_traffic_trend.py '{"asin":"B08GHW4TBS","campaignId":"SUBD"}'
```
