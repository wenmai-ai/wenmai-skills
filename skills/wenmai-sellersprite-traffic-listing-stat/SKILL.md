---
name: wenmai-sellersprite-traffic-listing-stat
description: "卖家精灵（SellerSprite） Amazon（亚马逊） 用于分析 Amazon ASIN 的流量来源结构，包括免费流量、付费流量和关联类型分布接口，通过固定 Wenmai standard API `traffic_listing_stat` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要用于分析 Amazon ASIN 的流量来源结构，包括免费流量、付费流量和关联类型分布或调用该接口时触发。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 卖家精灵（SellerSprite） standard API `traffic_listing_stat`。只调用以下固定端点，不接受动态端点或其他操作：
...
python3 scripts/traffic_listing_stat.py '{"asin":"B08GHW4TBS","marketplace":"US"}'
```
