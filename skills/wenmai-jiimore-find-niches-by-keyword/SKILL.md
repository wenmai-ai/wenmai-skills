...
description: "极目数据（JIIMORE）Amazon 关键词关联细分市场接口，按搜索词返回 niche 名称、需求、价格区间、商品/品牌数量、多周期搜索量/销量/点击、转化率、集中度、广告占比、新品成功率、退货率、CPC 和趋势。用于按词找 niche、市场发现、类目机会或细分赛道研究。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_find_niches_by_keyword`。只调用以下固定端点，不接受动态端点或其他操作：
