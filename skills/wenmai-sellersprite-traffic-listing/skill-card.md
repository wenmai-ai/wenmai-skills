...
卖家精灵（SellerSprite） Amazon 关联商品接口，用于查询指定 ASIN 在站内关联的 Listing，分析商品间的竞品和流量关联关系。

固定调用 Wenmai 卖家精灵（SellerSprite） standard API `traffic_listing`，不接受动态端点或其他操作。

- Provider: 卖家精灵（SellerSprite）
- Platform: Amazon（亚马逊）
- Operation: `traffic_listing`
