# Wenmai JIIMORE jiimore_get_keyword_rankings API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/jiimore/keyword-rankings`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，也兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`jiimore_get_keyword_rankings`
- **脚本入口**：`scripts/jiimore_get_keyword_rankings.py`
...
| `request.asin` | string | 是 | 单个 10 位字母数字 ASIN；服务端会去除首尾空格并转为大写。 |
| `request.countryCode` | string | 否 | 两位 Amazon 市场国家码，例如 US、JP、DE；省略时使用服务端默认市场。 |
| `request.days` | integer (7/30) | 否 | 关键词排名回溯窗口，只支持 7 或 30，默认 7。 |
...
| `items[].flowScore` | number | 流量分数，关键词排名接口返回。 |
| `items[].organicRank` | integer | 自然排名，关键词排名接口返回。 |
| `items[].overallOrganicRank` | integer | 整体自然排名，关键词排名接口返回。 |
| `items[].firstPageAsinCount` | integer | 首页命中 ASIN 数量。 |
| `items[].top16AsinCount` | integer | Top 16 命中 ASIN 数量。 |
| `items[].lastSpiderDate` | integer | 数据抓取日期，格式 yyyyMMdd。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/jiimore/keyword-rankings" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"request": {"asin": "B09PCSR9SX", "countryCode": "US", "days": 7}}'
```
