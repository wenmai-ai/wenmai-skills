...
description: "极目数据（JIIMORE）Amazon ASIN 关键词排名接口，返回搜索词、7 日搜索/点击/销量与转化、CPC/CPR、ABA 排名变化、流量分数、自然排名、首页/Top 16 命中数和抓取日期，支持 7 或 30 日回溯窗口。用于 ASIN 关键词排名、自然位、流量词、竞品排名或排名变化分析。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_get_keyword_rankings`。只调用以下固定端点，不接受动态端点或其他操作：
...
- 必须保留 `request` 包装层。
- `request.asin` 必填，必须是单个 10 位字母数字 ASIN。
- `request.days` 可选，只支持 7 或 30，省略时由服务端使用默认值 7。
...
- 按搜索词展示搜索/点击/销量与转化、CPC/CPR、ABA 排名变化、流量分数、自然排名、首页/Top 16 命中数和 `lastSpiderDate`。
