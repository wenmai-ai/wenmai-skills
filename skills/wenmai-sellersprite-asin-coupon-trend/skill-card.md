...
卖家精灵（SellerSprite） Amazon（亚马逊） 查询指定 ASIN 在 Amazon 指定市场下的优惠价格信息接口，通过固定 Wenmai standard API `asin_coupon_trend` 返回可追溯的原始网关数据。

固定调用 Wenmai 卖家精灵（SellerSprite） standard API `asin_coupon_trend`，不接受动态端点或其他操作。
