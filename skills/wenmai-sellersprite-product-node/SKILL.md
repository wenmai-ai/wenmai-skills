---
name: wenmai-sellersprite-product-node
description: "卖家精灵（SellerSprite） Amazon（亚马逊） 查询 Amazon 产品类目信息的工具接口，通过固定 Wenmai standard API `product_node` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要查询 Amazon 产品类目信息的工具或调用该接口时触发。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 卖家精灵（SellerSprite） standard API `product_node`。只调用以下固定端点，不接受动态端点或其他操作：
...
python3 scripts/product_node.py '{"request":{"marketplace":"US"}}'
```
