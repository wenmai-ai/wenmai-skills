---
name: wenmai-alpha-free-amazon-product-scraper
description: "Use when users need the Wenmai Alpha standard API `alpha_free_amazon_product_scraper` for Free Amazon Product Scraper, including Amazon 商品、评论、Listing、竞品或搜索结果. Sends user parameters as a JSON POST body to `/wmapi/v1/alpha/free-amazon-product-scraper` and returns traceable raw gateway data for ecommerce research, competitor discovery, VOC, social trend analysis, or reporting."
---

# Wenmai Alpha Free Amazon Product Scraper

## Purpose
Use this Skill to fetch Free Amazon Product Scraper data through Wenmai standard API. It is useful for Amazon 商品、评论、Listing、竞品或搜索结果.

The Skill calls exactly one fixed standard API endpoint:

- Endpoint: `POST /wmapi/v1/alpha/free-amazon-product-scraper`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/alpha_free_amazon_product_scraper.py`
...
python3 scripts/alpha_free_amazon_product_scraper.py '{"categoryUrls": [{"url": "https://www.amazon.com/s?k=keyboard"}], "useCaptchaSolver": false, "maxItemsPerStartUrl": 3, "scrapeProductDetails": true, "maxSearchPagesPerStartUrl": 3, "scrapeProductVariantPrices": false, "maxProductVariantsAsSeparateResults": 0}'
```
...
Required top-level fields: `categoryUrls`.
