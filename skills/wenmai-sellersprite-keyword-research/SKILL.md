---
name: wenmai-sellersprite-keyword-research
description: "卖家精灵（SellerSprite） Amazon（亚马逊） 专业级 Amazon 关键词市场与选品分析工具接口，通过固定 Wenmai standard API `keyword_research` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要专业级 Amazon 关键词市场与选品分析工具或调用该接口时触发。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# 卖家精灵（SellerSprite） keyword_research

## Overview

调用 Wenmai 卖家精灵（SellerSprite） standard API `keyword_research`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sellersprite/keyword-research`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/keyword_research.py`
...
python3 scripts/keyword_research.py '{"request":{"marketplace":"US"}}'
```
