...
卖家精灵（SellerSprite） Amazon（亚马逊） 专业级 Amazon 关键词市场与选品分析工具接口，通过固定 Wenmai standard API `keyword_research` 返回可追溯的原始网关数据。

固定调用 Wenmai 卖家精灵（SellerSprite） standard API `keyword_research`，不接受动态端点或其他操作。
