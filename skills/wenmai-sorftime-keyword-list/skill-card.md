...
Sorftime Amazon（亚马逊） 实时热搜关键词榜接口，通过固定 Wenmai standard API `keyword_list` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `keyword_list`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Amazon（亚马逊）
- Operation: `keyword_list`
