...
Sorftime Amazon（亚马逊） 细分类目（品类）市场接口，通过固定 Wenmai standard API `search_categories_broadly` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `search_categories_broadly`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Amazon（亚马逊）
- Operation: `search_categories_broadly`
