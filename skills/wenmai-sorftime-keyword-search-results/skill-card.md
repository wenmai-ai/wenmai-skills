...
Sorftime Amazon（亚马逊） 关键词搜索结果接口，通过固定 Wenmai standard API `keyword_search_results` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `keyword_search_results`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Amazon（亚马逊）
- Operation: `keyword_search_results`
