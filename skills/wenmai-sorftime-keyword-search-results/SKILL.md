---
name: wenmai-sorftime-keyword-search-results
description: "Sorftime Amazon（亚马逊） 关键词搜索结果接口，通过固定 Wenmai standard API `keyword_search_results` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要关键词搜索结果或调用该接口时触发。即使用户未明确提及 Sorftime，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# Sorftime Amazon（亚马逊） keyword_search_results

## Overview

调用 Wenmai Sorftime standard API `keyword_search_results`。只调用以下固定端点，不接受动态端点或其他操作：

- Platform: Amazon（亚马逊）
- Endpoint: `POST /wmapi/v1/sorftime/keyword-search-results`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/keyword_search_results.py`
...
python3 scripts/keyword_search_results.py '{"keyword":"wireless earbuds","keyword_support_site":"US"}'
```
