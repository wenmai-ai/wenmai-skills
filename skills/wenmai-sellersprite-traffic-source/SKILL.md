...
description: "卖家精灵（SellerSprite） Amazon 流量来源分析接口，用于从 ASIN 或关键词维度查看流量关键词结构和接口支持的来源指标。当用户提到流量来源、ASIN 流量结构、关键词来源、自然流量与广告流量、竞品流量入口或 SellerSprite traffic source 时触发此技能。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 卖家精灵（SellerSprite） standard API `traffic_source`。只调用以下固定端点，不接受动态端点或其他操作：
