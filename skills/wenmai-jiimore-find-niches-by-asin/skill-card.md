...
极目数据（JIIMORE）Amazon ASIN 关联细分市场接口，按单个 ASIN 返回 niche 名称、需求、价格区间、商品/品牌数量、多周期搜索量/销量/点击、转化率、集中度、广告占比、新品成功率、退货率、CPC 和趋势。

固定调用 Wenmai 极目数据（JIIMORE） standard API `jiimore_find_niches_by_asin`，不接受动态端点或其他操作。

- Provider: 极目数据（JIIMORE）
- Platform: Amazon（亚马逊）
- Operation: `jiimore_find_niches_by_asin`
