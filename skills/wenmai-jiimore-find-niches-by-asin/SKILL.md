...
description: "极目数据（JIIMORE）Amazon ASIN 关联细分市场接口，按单个 ASIN 返回 niche 名称、需求、价格区间、商品/品牌数量、多周期搜索量/销量/点击、转化率、集中度、广告占比、新品成功率、退货率、CPC 和趋势。用于 ASIN 市场归属、竞品赛道映射或细分机会分析。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# 极目数据（JIIMORE）Amazon ASIN 关联细分市场

## Overview

调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_find_niches_by_asin`。只调用以下固定端点，不接受动态端点或其他操作：
...
- 按 niche 展示名称、需求、价格区间、商品/品牌数量、多周期量级、转化、集中度、广告、新品、退货、CPC 和趋势。
