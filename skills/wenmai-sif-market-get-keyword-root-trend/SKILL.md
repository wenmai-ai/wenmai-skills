---
name: wenmai-sif-market-get-keyword-root-trend
description: "SIF Amazon（亚马逊） 需求边界层——回答'这个词背后的整个市场有多大，买家需求是集中在精确词上，还是分散在大量长尾变体词里'接口，通过固定 Wenmai standard API `market_get_keyword_root_trend` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要需求边界层——回答'这个词背后的整个市场有多大，买家需求是集中在精确词上，还是分散在大量长尾变体词里'或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai SIF standard API `market_get_keyword_root_trend`。只调用以下固定端点，不接受动态端点或其他操作：
...
python3 scripts/market_get_keyword_root_trend.py '{"keyword":"wireless earbuds"}'
```
