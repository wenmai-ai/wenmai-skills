...
description: "极目数据（JIIMORE）Amazon ASIN ABA 关键词接口，支持输入 1～20 个 ASIN，返回搜索词、搜索/点击/销量与转化、CPC/CPR、ABA 排名变化、集中度、相关度、竞争度、Top ASIN 和广告成本信号。用于 ASIN ABA 词、点击关键词、竞品流量来源、ABA 反查或广告机会分析。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_search_aba_keywords`。只调用以下固定端点，不接受动态端点或其他操作：
...
- 按搜索词展示量级、转化、CPC/CPR、ABA 排名变化、集中度、相关度、竞争度、Top ASIN 和广告成本信号。
