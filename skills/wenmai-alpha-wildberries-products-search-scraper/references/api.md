# Wenmai Alpha Wildberries Products Search Scraper API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/wildberries-products-search-scraper`
...
- **接口编码**：`alpha_wildberries_products_search_scraper`
- **接口说明**：Empty JavaScript project
- **脚本入口**：`scripts/alpha_wildberries_products_search_scraper.py`，脚本参数即标准 API POST Body JSON
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/wildberries-products-search-scraper" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"maxItems": 3, "searchUrl": "https://www.wildberries.ru/catalog/0/search.aspx?search=iphone"}'
```
