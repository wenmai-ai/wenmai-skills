---
name: wenmai-sorftime-tiktok-product-video
description: "Sorftime TikTok 产品带货视频接口，通过固定 Wenmai standard API `tiktok_product_video` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要产品带货视频或调用该接口时触发。即使用户未明确提及 Sorftime，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# Sorftime tiktok_product_video

## Overview

调用 Wenmai Sorftime standard API `tiktok_product_video`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sorftime/tiktok-product-video`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/tiktok_product_video.py`
...
python3 scripts/tiktok_product_video.py '{"product_id":"1732349647191642367","site":"US"}'
```
