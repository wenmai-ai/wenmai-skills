...
卖家精灵（SellerSprite） Amazon（亚马逊） 用于在指定 Amazon 站点中，根据 ASIN、时间范围及多维筛选条件，接口，通过固定 Wenmai standard API `traffic_extend` 返回可追溯的原始网关数据。

固定调用 Wenmai 卖家精灵（SellerSprite） standard API `traffic_extend`，不接受动态端点或其他操作。

- Provider: 卖家精灵（SellerSprite）
- Platform: Amazon（亚马逊）
- Operation: `traffic_extend`
