# Wenmai XYDC asin广告信息变动趋势（天） API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-asin-ad-change-trends`
...
- **接口编码**：`get_asin_ad_change_trends`
- **接口说明**：asin广告信息变动趋势（天）。
- **脚本入口**：`scripts/xydc_asin_ad_change_trends.py`，脚本参数即标准 API POST Body JSON
...
| `data.trends` | Array | 趋势。 |
| `data.trends[].date` | string | 趋势开始日期（YYYY-MM-DD格式）。 |
| `data.trends[].added` | Array | 新增广告活动。 |
| `data.trends[].added[].campaignId` | string | 广告活动id。 |
| `data.trends[].added[].campaignName` | string | 广告活动名。 |
| `data.trends[].added[].campaignType` | string | 广告活动类型。 |
| `data.trends[].removed` | Array | 停止广告活动。 |
| `data.trends[].removed[].campaignId` | string | 广告活动id。 |
| `data.trends[].removed[].campaignName` | string | 广告活动名。 |
| `data.trends[].removed[].campaignType` | string | 广告活动类型。 |

## 使用要点

- 本 Skill 直接调用 Wenmai 标准 API `/xydc/get-asin-ad-change-trends`。
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-asin-ad-change-trends" \
...
---
来源：Wenmai WMAPI 文档 https://all-api.wenmai-ai.com/wmapi/docs#xydc-get_asin_ad_change_trends（2026-07-07 访问）。
