...
description: "卖家精灵（SellerSprite） Amazon 类目市场研究接口，用于分析市场规模、商品数量、销量、销售额、均价、评分、BSR、品牌与卖家集中度、新品占比和配送结构。当用户提到类目市场分析、市场容量、竞争集中度、新品机会、FBA 占比、选品赛道或 SellerSprite market research 时触发此技能。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 卖家精灵（SellerSprite） standard API `market_research`。只调用以下固定端点，不接受动态端点或其他操作：
