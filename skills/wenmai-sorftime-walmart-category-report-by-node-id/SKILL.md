---
name: wenmai-sorftime-walmart-category-report-by-node-id
description: "Sorftime Walmart 类目实时销量报告接口，通过固定 Wenmai standard API `walmart_category_report_by_node_id` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要类目实时销量报告或调用该接口时触发。即使用户未明确提及 Sorftime，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# Sorftime walmart_category_report_by_node_id

## Overview

调用 Wenmai Sorftime standard API `walmart_category_report_by_node_id`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sorftime/walmart-category-report-by-node-id`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/walmart_category_report_by_node_id.py`
...
python3 scripts/walmart_category_report_by_node_id.py '{"node_id":"1055398"}'
```
