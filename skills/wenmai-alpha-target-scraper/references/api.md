# Wenmai Alpha Target Scraper API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/target-scraper`
...
- **接口编码**：`alpha_target_scraper`
- **接口说明**：Target Scraper
- **脚本入口**：`scripts/alpha_target_scraper.py`，脚本参数即标准 API POST Body JSON
...
| `maxProductsPerSearch` | integer | 否 | 最大采集或返回数量。 |
| `maxSearchPages` | integer | 否 | 最大采集或返回数量。 |
| `sort` | string | 否 | 排序方式。可选值：relevance、price_low、price_high、bestselling、newest。 |
...
| `title` | string | 标题。 |
| `price` | number | 价格。 |
| `priceString` | number | 价格。 |
| `regularPrice` | number | 价格。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/target-scraper" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"sort": "relevance", "searchQueries": ["laptop"], "maxSearchPages": 1, "maxRequestRetries": 3, "maxProductsPerSearch": 3}'
```
