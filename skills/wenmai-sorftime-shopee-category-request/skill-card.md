...
Sorftime Shopee 类目Best Seller查询接口，通过固定 Wenmai standard API `shopee_category_request` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `shopee_category_request`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Shopee
- Operation: `shopee_category_request`
