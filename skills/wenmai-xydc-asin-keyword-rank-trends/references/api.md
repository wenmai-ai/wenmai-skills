# Wenmai XYDC asin词排名趋势（天） API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-asin-keyword-rank-trends`
...
- **接口编码**：`get_asin_keyword_rank_trends`
- **接口说明**：asin词排名趋势（天）。
- **脚本入口**：`scripts/xydc_asin_keyword_rank_trends.py`，脚本参数即标准 API POST Body JSON
...
- 本 Skill 直接调用 Wenmai 标准 API `/xydc/get-asin-keyword-rank-trends`。
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-asin-keyword-rank-trends" \
...
---
来源：Wenmai WMAPI 文档 https://all-api.wenmai-ai.com/wmapi/docs#xydc-get_asin_keyword_rank_trends（2026-07-07 访问）。
