---
name: wenmai-sif-ads-get-asin-ad-structure
description: "SIF Amazon（亚马逊） 功能：查询某 ASIN 的广告结构总览，统计历史全量范围内各广告类型的 campaign 数量接口，通过固定 Wenmai standard API `ads_get_asin_ad_structure` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要功能：查询某 ASIN 的广告结构总览，统计历史全量范围内各广告类型的 campaign 数量或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai SIF standard API `ads_get_asin_ad_structure`。只调用以下固定端点，不接受动态端点或其他操作：
...
python3 scripts/ads_get_asin_ad_structure.py '{"asin":"B08GHW4TBS","country":"US"}'
```
