...
description: "卖家精灵（SellerSprite） ASIN 流量关键词概览统计接口，用于汇总自然词、广告词、流量词和转化词等关键词结构指标。当用户提到关键词结构、流量词统计、自然与广告关键词占比、ASIN 关键词概览或 traffic keyword stat 时触发此技能。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 卖家精灵（SellerSprite） standard API `traffic_keyword_stat`。只调用以下固定端点，不接受动态端点或其他操作：
