---
name: wenmai-sif-ops-get-asin-sales-list
description: "SIF Amazon（亚马逊） 功能：以列表视图查询一个或多个 ASIN 的销量数据，返回各变体的销量、价格、属性及月度趋势迷你图接口，通过固定 Wenmai standard API `ops_get_asin_sales_list` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要功能：以列表视图查询一个或多个 ASIN 的销量数据，返回各变体的销量、价格、属性及月度趋势迷你图或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai SIF standard API `ops_get_asin_sales_list`。只调用以下固定端点，不接受动态端点或其他操作：
...
python3 scripts/ops_get_asin_sales_list.py '{"asins":["B08GHW4TBS"]}'
```
