# Wenmai JIIMORE jiimore_find_same_niche_asins API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/jiimore/same-niche-asins`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，也兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`jiimore_find_same_niche_asins`
- **脚本入口**：`scripts/jiimore_find_same_niche_asins.py`
...
| `items[].currentPrice` | number | 当前价格。 |
...
| `items[].count` | integer | 该商品命中的同细分市场计数。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/jiimore/same-niche-asins" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"request": {"asin": "B09PCSR9SX", "countryCode": "US"}}'
```
