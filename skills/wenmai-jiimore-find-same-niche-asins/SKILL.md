...
description: "极目数据（JIIMORE）Amazon 同细分市场商品接口，按单个 ASIN 查找同一 niche 的商品，并返回父 ASIN、价格、评分、评论量、7/30/360 日点击与转化及同市场命中计数。用于同 niche 竞品、相同赛道 ASIN、竞品扩展或细分市场对标。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_find_same_niche_asins`。只调用以下固定端点，不接受动态端点或其他操作：
...
- 按 ASIN 展示父 ASIN、价格、评分/评论、点击与转化，以及同细分市场命中计数。
