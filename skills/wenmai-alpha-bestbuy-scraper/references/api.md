# Wenmai Alpha Bestbuy Scraper API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/bestbuy-scraper`
...
- **接口编码**：`alpha_bestbuy_scraper`
- **接口说明**：Best Buy Product Scraper
- **脚本入口**：`scripts/alpha_bestbuy_scraper.py`，脚本参数即标准 API POST Body JSON
...
| `fetchProductDetails` | boolean | 否 | 是否启用该选项。 |
| `zipCode` | string | 否 | zipCode 字段。 |
...
| `fetchProductDetails` | boolean | 是否启用该选项。 |
| `zipCode` | string | zipCode 字段。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/bestbuy-scraper" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"zipCode": "10001", "maxResults": 3, "searchQuery": "laptop", "fetchProductDetails": true}'
```
