...
Sorftime Amazon（亚马逊） 查询收藏关键词接口，通过固定 Wenmai standard API `get_favorite_keyword` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `get_favorite_keyword`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Amazon（亚马逊）
- Operation: `get_favorite_keyword`
