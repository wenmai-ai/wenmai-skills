...
卖家精灵（SellerSprite） Amazon（亚马逊） 高级关键词流量与竞争分析工具（卖家决策级）接口，通过固定 Wenmai standard API `keyword_miner` 返回可追溯的原始网关数据。

固定调用 Wenmai 卖家精灵（SellerSprite） standard API `keyword_miner`，不接受动态端点或其他操作。
