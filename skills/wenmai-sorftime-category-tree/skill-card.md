...
Sorftime Amazon（亚马逊） 类目树查询接口，通过固定 Wenmai standard API `category_tree` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `category_tree`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Amazon（亚马逊）
- Operation: `category_tree`
