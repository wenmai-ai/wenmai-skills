# Wenmai XYDC asin商品信息 API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-asin-info`
...
- **接口编码**：`get_asin_info`
- **接口说明**：asin商品信息。
- **脚本入口**：`scripts/xydc_asin_info.py`，脚本参数即标准 API POST Body JSON
...
| `data.entities[].amazonUrl` | string | ASIN的listing链接。 |
| `data.entities[].bigPicUrl` | string | ASIN主图url（大尺寸_512px）。 |
| `data.entities[].currency` | string | 货币代码。 |
| `data.entities[].price` | string | ASIN价格。 |
| `data.entities[].ratings` | integer | Ratings数量。 |
| `data.entities[].stars` | string | Ratings评分。 |
| `data.entities[].title` | string | ASIN标题。 |
| `data.entities[].smallPicUrl` | string | ASIN主图url（小尺寸_128px）。 |

## 使用要点

- 本 Skill 直接调用 Wenmai 标准 API `/xydc/get-asin-info`。
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-asin-info" \
...
---
来源：Wenmai WMAPI 文档 https://all-api.wenmai-ai.com/wmapi/docs#xydc-get_asin_info（2026-07-07 访问）。
