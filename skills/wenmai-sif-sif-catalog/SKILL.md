---
name: wenmai-sif-sif-catalog
description: "SIF Amazon（亚马逊） 返回 SIF 可用接口的分类目录。触发时机：用户询问'有哪些工具' / '能做什么' / '功能列表' / '工具介绍' 时使用。返回内容按大类组织接口，通过固定 Wenmai standard API `sif_catalog` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要返回 SIF 可用接口的分类目录。触发时机：用户询问'有哪些工具' / '能做什么' / '功能列表' / '工具介绍' 时使用。返回内容按大类组织或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai SIF standard API `sif_catalog`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sif/sif-catalog`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/sif_catalog.py`
...
python3 scripts/sif_catalog.py '{}'
```
