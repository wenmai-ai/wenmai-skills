---
name: wenmai-sellersprite-market-listing-date-distribution
description: "卖家精灵（SellerSprite） Amazon（亚马逊） 用于分析指定 Amazon 市场类目节点下的商品上架时间分布与新品接受度接口，通过固定 Wenmai standard API `market_listing_date_distribution` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要用于分析指定 Amazon 市场类目节点下的商品上架时间分布与新品接受度或调用该接口时触发。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# 卖家精灵（SellerSprite） market_listing_date_distribution

## Overview

调用 Wenmai 卖家精灵（SellerSprite） standard API `market_listing_date_distribution`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sellersprite/market-listing-date-distribution`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/market_listing_date_distribution.py`
...
python3 scripts/market_listing_date_distribution.py '{"request":{"nodeIdPath":"172282:281407","marketplace":"US"}}'
```
