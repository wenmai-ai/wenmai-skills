...
Sorftime Temu 按名称搜产品接口，通过固定 Wenmai standard API `temu_product_search_from_name` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `temu_product_search_from_name`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Temu
- Operation: `temu_product_search_from_name`
