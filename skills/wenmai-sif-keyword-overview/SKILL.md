...
description: "SIF Amazon 关键词需求概览接口，用于查询搜索量历史、ABA 排名历史、Top 3 点击或转化集中度、市场需求规模和关键词趋势。当用户提到关键词搜索量、ABA 排名、点击集中度、转化集中度、需求趋势或 SIF keyword overview 时触发此技能。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai SIF standard API `market_get_keyword_history`。只调用以下固定端点，不接受动态端点或其他操作：
