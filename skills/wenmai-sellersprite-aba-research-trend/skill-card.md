...
卖家精灵（SellerSprite） Amazon（亚马逊） ABA选品-关键词的趋势数据，包含：ABA排名和搜索量接口，通过固定 Wenmai standard API `aba_research_trend` 返回可追溯的原始网关数据。

固定调用 Wenmai 卖家精灵（SellerSprite） standard API `aba_research_trend`，不接受动态端点或其他操作。

- Provider: 卖家精灵（SellerSprite）
- Platform: Amazon（亚马逊）
- Operation: `aba_research_trend`
