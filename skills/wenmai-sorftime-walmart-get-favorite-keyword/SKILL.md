---
name: wenmai-sorftime-walmart-get-favorite-keyword
description: "Sorftime Walmart 查询收藏关键词接口，通过固定 Wenmai standard API `walmart_get_favorite_keyword` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要查询收藏关键词或调用该接口时触发。即使用户未明确提及 Sorftime，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# Sorftime walmart_get_favorite_keyword

## Overview

调用 Wenmai Sorftime standard API `walmart_get_favorite_keyword`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sorftime/walmart-get-favorite-keyword`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/walmart_get_favorite_keyword.py`
...
python3 scripts/walmart_get_favorite_keyword.py '{}'
```
...
此操作会修改远程状态。执行前必须向用户复核最终完整请求，并确认全部目标/作用域字段：`dict`, `page`。可选目标字段未提供时，也必须确认采用上游默认作用域；未确认时不要调用。
