...
Sorftime TikTok 按产品名称搜类目接口，通过固定 Wenmai standard API `tiktok_category_name_search` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `tiktok_category_name_search`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: TikTok
- Operation: `tiktok_category_name_search`
