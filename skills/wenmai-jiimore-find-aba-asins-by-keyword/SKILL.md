...
description: "极目数据（JIIMORE）Amazon ABA 关键词关联商品接口，支持按 1～20 个关键词查找 ASIN，并返回标题、图片、品牌、价格、评分、评论量、7/30/360 日点击与转化、卖家及 ABA 相似度/相关等级。用于按词找 ASIN、ABA 头部商品、竞品发现或关键词关联商品分析。即使用户未明确提及 JIIMORE，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# 极目数据（JIIMORE）Amazon ABA 关键词关联商品

## Overview

调用 Wenmai 极目数据（JIIMORE）standard API `jiimore_find_aba_asins_by_keyword`。只调用以下固定端点，不接受动态端点或其他操作：
...
- 必须保留 `request` 包装层。
- `request.keywords` 必填，必须是 1～20 个关键词；每个关键词最多 200 字符，超限时明确分批，不静默截断。
...
- 按 ASIN 展示商品信息、价格、评分/评论、点击与转化、卖家，以及存在时的 ABA 相似度和相关等级。
