---
name: wenmai-sif-market-get-keyword-demand
description: "SIF Amazon（亚马逊） 需求判断层——回答'这个词的需求处于什么生命周期阶段，是在增长、在萎缩还是只是季节性低谷，以及现在是进场、加速、收割还是收缩的时机'接口，通过固定 Wenmai standard API `market_get_keyword_demand` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要需求判断层——回答'这个词的需求处于什么生命周期阶段，是在增长、在萎缩还是只是季节性低谷，以及现在是进场、加速、收割还是收缩的时机'或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai SIF standard API `market_get_keyword_demand`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sif/market-get-keyword-demand`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/market_get_keyword_demand.py`
...
python3 scripts/market_get_keyword_demand.py '{"keywords":["wireless earbuds"]}'
```
