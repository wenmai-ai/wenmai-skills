...
SIF Amazon（亚马逊） 需求判断层——回答'这个词的需求处于什么生命周期阶段，是在增长、在萎缩还是只是季节性低谷，以及现在是进场、加速、收割还是收缩的时机'接口，通过固定 Wenmai standard API `market_get_keyword_demand` 返回可追溯的原始网关数据。

固定调用 Wenmai SIF standard API `market_get_keyword_demand`，不接受动态端点或其他操作。

- Provider: SIF
- Platform: Amazon（亚马逊）
- Operation: `market_get_keyword_demand`
