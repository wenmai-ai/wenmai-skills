...
Sorftime Amazon（亚马逊） 潜力产品搜索接口，通过固定 Wenmai standard API `potential_product` 返回可追溯的原始网关数据。

固定调用 Wenmai Sorftime standard API `potential_product`，不接受动态端点或其他操作。

- Provider: Sorftime
- Platform: Amazon（亚马逊）
- Operation: `potential_product`
