...
卖家精灵（SellerSprite） Amazon（亚马逊） 关键词选品-关键词的趋势数据，包含：搜索量，购买量，购买率，同比增长率，环比增长率，三个月增长率接口，通过固定 Wenmai standard API `keyword_research_trends` 返回可追溯的原始网关数据。

固定调用 Wenmai 卖家精灵（SellerSprite） standard API `keyword_research_trends`，不接受动态端点或其他操作。
