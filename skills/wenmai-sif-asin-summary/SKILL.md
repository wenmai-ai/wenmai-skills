...
description: "SIF ASIN 流量概览接口，用于汇总 Listing 的自然与广告流量占比、SP／SB／SBV 渠道结构、推荐流量来源和整体流量健康度。当用户提到 ASIN 流量概览、自然广告占比、广告渠道结构、Listing 流量健康或 SIF ASIN summary 时触发此技能。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---
...
调用 Wenmai SIF standard API `ops_get_listing_traffic_overview`。只调用以下固定端点，不接受动态端点或其他操作：
