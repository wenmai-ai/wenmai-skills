# Wenmai Alpha Walmart Fast Product Scraper API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/walmart-fast-product-scraper`
...
- **接口编码**：`alpha_walmart_fast_product_scraper`
- **接口说明**：Walmart Fast Product Scraper
- **脚本入口**：`scripts/alpha_walmart_fast_product_scraper.py`，脚本参数即标准 API POST Body JSON
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/walmart-fast-product-scraper" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"startUrls": [{"url": "https://walmart.com/search?q=tshirt"}], "maxProductsPerStartUrl": 3}'
```
