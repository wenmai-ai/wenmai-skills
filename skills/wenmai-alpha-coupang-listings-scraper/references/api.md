# Wenmai Alpha Coupang Listings Scraper API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/coupang-listings-scraper`
...
- **接口编码**：`alpha_coupang_listings_scraper`
- **接口说明**：Coupang Listings Scraper (South Korea)
- **脚本入口**：`scripts/alpha_coupang_listings_scraper.py`，脚本参数即标准 API POST Body JSON
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `searchTerms` | array | 搜索关键词列表。 |
| `startUrls` | array | 页面链接列表。 |
| `productUrls` | array | 页面链接列表。 |
| `maxResults` | integer | 最大采集或返回数量。 |

## 使用要点

- 本接口适合：Coupang 韩国电商商品。
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/coupang-listings-scraper" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"maxResults": 3, "searchTerms": ["laptop"]}'
```
