---
name: wenmai-sorftime-get-favorite-keyword-dict
description: "Sorftime Amazon（亚马逊） 查询收藏夹列表接口，通过固定 Wenmai standard API `get_favorite_keyword_dict` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要查询收藏夹列表或调用该接口时触发。即使用户未明确提及 Sorftime，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# Sorftime Amazon（亚马逊） get_favorite_keyword_dict

## Overview

调用 Wenmai Sorftime standard API `get_favorite_keyword_dict`。只调用以下固定端点，不接受动态端点或其他操作：

- Platform: Amazon（亚马逊）
- Endpoint: `POST /wmapi/v1/sorftime/get-favorite-keyword-dict`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/get_favorite_keyword_dict.py`
...
python3 scripts/get_favorite_keyword_dict.py '{"keyword_support_site":"US"}'
```
...
此操作会修改远程状态。执行前必须向用户复核最终完整请求，并确认全部目标/作用域字段：`keyword_support_site`, `page`。可选目标字段未提供时，也必须确认采用上游默认作用域；未确认时不要调用。
