# Wenmai XYDC asin反查关键词列表（月） API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-asin-keywords-monthly`
...
- **接口编码**：`get_asin_keywords_monthly`
- **接口说明**：asin反查关键词列表（月）。
- **脚本入口**：`scripts/xydc_asin_keywords_monthly.py`，脚本参数即标准 API POST Body JSON
...
| `data.list[].trafficSummary.trafficAcquisitionRate` | object | 流量获得率。 |
...
- 本 Skill 直接调用 Wenmai 标准 API `/xydc/get-asin-keywords-monthly`。
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-asin-keywords-monthly" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"asin": "B09PCSR9SX", "country": "US", "start_month": "2026-05", "end_month": "2026-06", "page": 1, "page_size": 20}'
```

---
来源：Wenmai WMAPI 文档 https://all-api.wenmai-ai.com/wmapi/docs#xydc-get_asin_keywords_monthly（2026-07-07 访问）。
