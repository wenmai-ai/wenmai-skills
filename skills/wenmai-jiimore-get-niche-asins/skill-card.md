...
极目数据（JIIMORE）Amazon 细分市场商品接口，按 niche ID 返回商品 ASIN、标题、品牌、价格、评分、评论量、点击与转化、卖家，以及商品在细分市场中的 90 日点击份额。

固定调用 Wenmai 极目数据（JIIMORE） standard API `jiimore_get_niche_asins`，不接受动态端点或其他操作。

- Provider: 极目数据（JIIMORE）
- Platform: Amazon（亚马逊）
- Operation: `jiimore_get_niche_asins`
