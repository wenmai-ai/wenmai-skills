# Wenmai JIIMORE jiimore_get_niche_asins API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/jiimore/niche-asins`
- **请求方式**：POST，`Content-Type: application/json`
- **认证方式**：Header `secret-key: $WENMAI_API_KEY`，也兼容 `WENMAI_SECRET_KEY`
- **接口代码**：`jiimore_get_niche_asins`
- **脚本入口**：`scripts/jiimore_get_niche_asins.py`
...
| `items[].clickShare90d` | number | 商品在细分市场中的 90 日点击份额，条件字段。 |
| `items[].asinClickCount90d` | integer | 商品 90 日点击量，条件字段。 |
| `items[].nicheClickCount90d` | integer | 细分市场 90 日点击量，条件字段。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/jiimore/niche-asins" \
