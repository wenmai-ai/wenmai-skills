# Wenmai XYDC 关键词分析列表（最近天） API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-keyword-asin-analysis`
...
- **接口编码**：`get_keyword_asin_analysis`
- **接口说明**：关键词分析列表（最近天）。
- **脚本入口**：`scripts/xydc_keyword_asin_analysis.py`，脚本参数即标准 API POST Body JSON
...
| `keyword` | string | 是 | 关键词；示例：`usb c cable`。 |
...
- 本 Skill 直接调用 Wenmai 标准 API `/xydc/get-keyword-asin-analysis`。
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/xydc/get-keyword-asin-analysis" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"keyword": "usb c cable", "country": "US", "page": 1, "page_size": 20}'
```

---
来源：Wenmai WMAPI 文档 https://all-api.wenmai-ai.com/wmapi/docs#xydc-get_keyword_asin_analysis（2026-07-07 访问）。
