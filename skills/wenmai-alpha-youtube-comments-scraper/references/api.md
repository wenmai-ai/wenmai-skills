# Wenmai Alpha Youtube Comments Scraper API 参考

## 调用规范

- **请求地址**：`${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/youtube-comments-scraper`
...
- **接口编码**：`alpha_youtube_comments_scraper`
- **接口说明**：Youtube Scraper
- **脚本入口**：`scripts/alpha_youtube_comments_scraper.py`，脚本参数即标准 API POST Body JSON
...
| `sortCommentsBy` | string | 否 | 排序方式。可选值：TOP_COMMENTS、NEWEST_FIRST。 |
| `oldestCommentDate` | string | 否 | 时间或日期。 |
...
|------|------|------|
| `cid` | string | ID 标识。 |
| `replyToCid` | string | ID 标识。 |
| `type` | string | 采集模式。 |
| `publishedTimeText` | string | 时间或日期。 |
| `comment` | string | 评论列表。 |
| `author` | string | 作者信息。 |
| `authorIsChannelOwner` | boolean | 作者信息。 |
| `replyCount` | integer | 数量。 |
| `voteCount` | integer | 数量。 |
| `hasCreatorHeart` | boolean | 是否启用该选项。 |
| `videoId` | string | ID 标识。 |
| `pageUrl` | string | 链接地址。 |
| `commentsCount` | integer | 评论列表。 |
| `title` | string | 标题。 |
...
```bash
curl -sS -X POST \
  "${WENMAI_API_ORIGIN:-https://all-api.wenmai-ai.com}/wmapi/v1/alpha/youtube-comments-scraper" \
  -H "secret-key: $WENMAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"startUrls": [{"url": "https://www.youtube.com/watch?v=xObhZ0Ga7EQ"}], "maxComments": 3, "sortCommentsBy": "NEWEST_FIRST", "oldestCommentDate": "2026-03-01"}'
```
