---
name: wenmai-sif-analyze-traffic-anomaly
description: "SIF Amazon（亚马逊） 所有流量变化分析的主入口工具。功能：对 ASIN 进行端到端的流量下跌根因分析——自动识别异常窗口，从广告侧、自然侧、关键词侧逐层拆因。注意：判断逻辑仍在持续迭接口，通过固定 Wenmai standard API `analyze_traffic_anomaly` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要所有流量变化分析的主入口工具。功能：对 ASIN 进行端到端的流量下跌根因分析——自动识别异常窗口，从广告侧、自然侧、关键词侧逐层拆因。注意：判断逻辑仍在持续迭或调用该接口时触发。即使用户未明确提及 SIF，只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# SIF analyze_traffic_anomaly

## Overview

调用 Wenmai SIF standard API `analyze_traffic_anomaly`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sif/analyze-traffic-anomaly`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/analyze_traffic_anomaly.py`
...
python3 scripts/analyze_traffic_anomaly.py '{"asin":"B08GHW4TBS"}'
```
