---
name: wenmai-sellersprite-asin-prediction
description: "卖家精灵（SellerSprite） Amazon（亚马逊） 查询指定 ASIN 在 Amazon 对应市场的商品基础信息及销量与销售额预测数据接口，通过固定 Wenmai standard API `asin_prediction` 返回可追溯的原始网关数据。用于与该能力相关的数据查询、核验、分析或报告；当用户需要查询指定 ASIN 在 Amazon 对应市场的商品基础信息及销量与销售额预测数据或调用该接口时触发。即使用户未明确提及 卖家精灵（SellerSprite），只要任务需要上述接口能力，也应触发此技能。"
metadata:
  author: wenmai-ai
  version: "1.0.0"
---

# 卖家精灵（SellerSprite） asin_prediction

## Overview

调用 Wenmai 卖家精灵（SellerSprite） standard API `asin_prediction`。只调用以下固定端点，不接受动态端点或其他操作：

- Endpoint: `POST /wmapi/v1/sellersprite/asin-prediction`
- Auth: Header `secret-key: $WENMAI_API_KEY`
- Script: `scripts/asin_prediction.py`
...
python3 scripts/asin_prediction.py '{"asin":"B08GHW4TBS","marketplace":"US"}'
```
