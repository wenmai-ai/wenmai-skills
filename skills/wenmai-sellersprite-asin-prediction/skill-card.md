...
卖家精灵（SellerSprite） Amazon（亚马逊） 查询指定 ASIN 在 Amazon 对应市场的商品基础信息及销量与销售额预测数据接口，通过固定 Wenmai standard API `asin_prediction` 返回可追溯的原始网关数据。

固定调用 Wenmai 卖家精灵（SellerSprite） standard API `asin_prediction`，不接受动态端点或其他操作。

- Provider: 卖家精灵（SellerSprite）
- Platform: Amazon（亚马逊）
- Operation: `asin_prediction`
